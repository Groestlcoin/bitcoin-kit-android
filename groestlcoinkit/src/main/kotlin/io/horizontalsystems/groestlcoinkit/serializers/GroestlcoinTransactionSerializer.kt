package io.horizontalsystems.groestlcoinkit.serializers
...
object GroestlcoinTransactionSerializer {
    fun deserialize(input: BitcoinInput): FullTransaction {
        ...
        fullTransaction.header.hash = HashUtils.sha256(serialize(fullTransaction, withWitness = false))
    ...
    fun serializeForSignature(transaction: Transaction, inputsToSign: List<InputToSign>, outputs: List<TransactionOutput>, inputIndex: Int, isWitness: Boolean = false): ByteArray {
        ...
        if (isWitness) {
            ...
            buffer.write(HashUtils.sha256(outpoints.toByteArray())) // hash prevouts
            buffer.write(HashUtils.sha256(sequences.toByteArray())) // hash sequence
